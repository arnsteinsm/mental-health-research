...
const Footer: React.FC = () => {
	...
	const academicSources = evidenceSources.filter(
		(s) => s.type === "academic",
	).length;
	const institutionalSources = evidenceSources.filter(
		(s) => s.type === "institutional",
	).length;
	const governmentSources = evidenceSources.filter(
		(s) => s.type === "government",
	).length;
	const metaAnalysisSources = evidenceSources.filter(
	...
	const dataSources = [
		...
		},
		{
			...
			url: "#evidence-modal",
			description: `${evidenceCount} peer-reviewed sources (${academicSources} academic, ${metaAnalysisSources} meta-analysis, ${institutionalSources} institutional, ${governmentSources} government)`,
			onClick: () => {
	...
	return (
		<footer className="bg-gray-900 text-white py-12">
				...
				<div className="grid md:grid-cols-2 gap-12">
					...
					<div>
						...
						<p className="text-gray-400 text-sm leading-relaxed mb-6">
							What the data proves: alcohol misuse reflects deeper mental health
							struggles, especially for men.{" "}
						...
						<div className="space-y-4 mb-6">
							<div>
								...
								<ul className="text-gray-400 text-xs space-y-1">
									<li>
										• Strong correlation (r={correlations.overall.toFixed(2)})
										between alcohol and suicide mortality
									</li>
									<li>
										• Men show {genderRatio}x higher alcohol-related death rates
										than women
									</li>
									<li>
										• Consistent patterns across all {countryCount} European
										countries
									</li>
								</ul>
							</div>
				...
				>
					<div className="text-xs text-gray-500 text-center">
						<p>
							...
							for public health data analysis. All data sources are publicly
							available and properly attributed.
						</p>
					</div>
				</motion.div>
			</div>
			...
			<EvidenceModal
				...
				claimTitle="All Research Claims"
			/>
		</footer>
	);
};
