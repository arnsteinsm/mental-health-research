// apps/web/src/components/InteractiveVisualization.tsx
...
const _InteractiveVisualization: React.FC = () => {
	...
	const [viewMode, setViewMode] = useState<"comparison" | "timeseries">(
		"comparison",
	);
	const [genderView, setGenderView] = useState<"separate" | "combined">(
		"separate",
	);
	...
	// Use shared package data service (no calculations needed - all pre-computed)
	const {
		data: researchData = [],
	...
	const metrics = {
		alcohol_rate: {
			label: "Alcohol-Related Mortality Rate",
			color: "#8b5cf6",
			unit: "per 100k",
			description:
		...
		suicide_rate: {
			...
			unit: "per 100k",
			description:
				"Age-standardized suicide death rate (ICD-10: X60–X84, Y870)",
		},
		est_alcohol_deaths: {
			label: "Estimated Alcohol Deaths",
			color: "#8b5cf6",
			unit: "people (est.)",
			description:
				"Statistical estimates of people who died from alcohol-related causes - based on population data and mortality rates, not exact counts",
			isHumanImpact: true,
		},
		est_suicide_deaths: {
			label: "Estimated Suicide Deaths",
			color: "#ef4444",
			unit: "people (est.)",
			description:
				"Statistical estimates of people who died by suicide - calculated from official mortality data, represents approximate human impact",
			isHumanImpact: true,
		},
	};
	...
	const generateShareableUrl = () => {
		...
		return `${window.location.origin}${window.location.pathname}#interactive-visualization?${params.toString()}`;
	};

	// Copy shareable link to clipboard
	const _copyShareableLink = async () => {
		try {
	...
	// Handle URL parameters and scroll to section on load
	useEffect(() => {
		const hash = window.location.hash;
		if (hash.includes("#interactive-visualization")) {
			...
			if (view && ["comparison", "timeseries"].includes(view)) {
				setViewMode(view as typeof viewMode);
			}
			if (gender && ["separate", "combined"].includes(gender)) {
				setGenderView(gender as typeof genderView);
			}

			// Scroll to the section after a brief delay to ensure content is rendered
			setTimeout(() => {
				const element = document.getElementById("interactive-visualization");
				if (element) {
	...
	useEffect(() => {
		...
		if (genderView === "combined") {
			...
			const combinedData = selectedCountries
				.map((country) => {
					const maleData = filteredData.find(
						(d) => d.country === country && d.sex === "M",
					);
					const femaleData = filteredData.find(
						(d) => d.country === country && d.sex === "F",
					);

					if (!maleData || !femaleData) return null;

					// For estimated deaths: simply add male + female death counts from source data
					// For rates: use weighted average based on population
					let _combinedValue: number;
					if (
						selectedMetric === "est_alcohol_deaths" ||
						selectedMetric === "est_suicide_deaths"
					) {
						// Add actual death counts directly from source data - this is the truth!
						_combinedValue =
							Number.parseFloat(maleData[selectedMetric] || "0") +
							Number.parseFloat(femaleData[selectedMetric] || "0");
					} else {
						// Use weighted average for rates
					...
					return {
						...
						[selectedMetric]: _combinedValue.toString(),
						alcohol_rate: _combinedValue.toString(),
						suicide_rate: _combinedValue.toString(),
						accident_rate: "",
					};
				})
				...
				<div className="container mx-auto px-6">
					<div className="text-center">
						...
						<p className="text-gray-600 mb-4">
							There was an error loading the research data. Please try
							refreshing the page.
						</p>
						<button
							type="button"
	...
	return (
		<section id="interactive-visualization" className="py-20 bg-white">
			<div className="container mx-auto px-6">
				<motion.div
				...
				>
					...
					<p className="text-xl text-gray-600 max-w-4xl mx-auto">
						Explore the full dataset (2011 - 2022) across{" "}
						{availableCountries.length} European countries. Compare mortality
						rates and estimated actual death counts to understand the human
						impact.
					</p>
				</motion.div>
							...
							<div className="flex items-center justify-between">
								<div className="flex items-center space-x-2">
									...
									<span className="text-sm font-medium text-blue-800">
										Want personalized country suggestions based on your
										location?
									</span>
								</div>
								<button
					...
					{geoData && (
						...
						>
							<div className="flex items-center justify-between">
								...
								{geoData.isDetected &&
									geoData.suggestedCountries.length > 0 && (
										<button
											type="button"
											onClick={() => {
						...
						<div className="flex flex-wrap items-center gap-4">
							...
							>
								...
								<button
									...
									className="flex items-center gap-1 px-2 py-1 bg-white border border-gray-300 rounded text-sm hover:bg-gray-50 transition-colors"
								>
									<span className="text-gray-700">
										{selectedCountries.length === 0
											? "None"
											: selectedCountries.length === 1
												? `${countryNames[selectedCountries[0]] || selectedCountries[0]}`
												: `${selectedCountries.length} selected`}
									</span>
									<ChevronDown
										className={`w-3 h-3 text-gray-400 transition-transform ${isCountryDropdownOpen ? "rotate-180" : ""}`}
									/>
								</button>

								{/* Enhanced Dropdown */}
								{isCountryDropdownOpen && (
									<div className="absolute z-10 mt-1 bg-white border border-gray-300 rounded-lg shadow-lg w-80">
										{/* Search and Controls inside dropdown */}
										<div className="p-2 border-b border-gray-200 space-y-2">
											...
											<div className="relative">
												...
												<input
													...
													className="w-full pl-7 pr-2 py-1 border border-gray-300 rounded text-xs focus:ring-1 focus:ring-blue-500 focus:border-transparent"
												/>
											</div>
													...
													<div className="space-y-0.5">
														{filteredCountries.map((country) => (
															<label
																...
																className="flex items-center space-x-2 text-xs p-1 hover:bg-gray-50 rounded cursor-pointer"
															>
																<input
																	id={`country-${country}`}
																	type="checkbox"
																...
																{geoData?.suggestedCountries.includes(
																	country,
																) && (
							...
							{/* Year Selection - only show in comparison mode */}
							{viewMode === "comparison" && (
								<div className="flex items-center gap-2">
							...
							{viewMode === "comparison" && (
								<div className="flex items-center gap-2">
									...
									<select
										value={genderView}
										onChange={(e) =>
											setGenderView(e.target.value as typeof genderView)
										}
										className="px-2 py-1 border border-gray-300 rounded text-sm focus:ring-1 focus:ring-blue-500 focus:border-transparent"
									>
										<option value="separate">Male vs Female</option>
									...
									{hoveredData && (
										<div className="absolute top-4 right-4 bg-black/75 text-white p-2 rounded text-sm">
											<div className="font-medium">
												{countryNames[hoveredData.country] ||
													hoveredData.country}
											</div>
											<div>
												...
												selectedMetric === "est_suicide_deaths"
													? Math.round(
															Number.parseFloat(hoveredData[selectedMetric]),
														).toLocaleString()
													: Number.parseFloat(
...
export default _InteractiveVisualization;
