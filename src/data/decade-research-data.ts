...
const createDecadeDataset = (): DecadeDataPoint[] => {
  ...
  countries.forEach((country) => {
    ...
    const baseMaleAlcohol = actualMaleData
      ? Number.parseFloat(actualMaleData.alcohol_rate)
      : 15 + Math.random() * 20;
    const baseMaleSuicide = actualMaleData
      ? Number.parseFloat(actualMaleData.suicide_rate)
      : 20 + Math.random() * 30;
    const baseFemaleAlcohol = actualFemaleData
      ? Number.parseFloat(actualFemaleData.alcohol_rate)
      : baseMaleAlcohol * 0.3;
    const baseFemaleSuicide = actualFemaleData
      ? Number.parseFloat(actualFemaleData.suicide_rate)
      : baseMaleSuicide * 0.4;
