...
import { researchData, DataPoint } from './research-data';
...
export const verifiedStats = {
  ...
  yearCoverage: (() => {
    const yearCounts = researchData.reduce((acc, d) => {
  ...
  countryCoverage: (() => {
    const countryCounts = researchData.reduce((acc, d) => {
  ...
  genderRatios: (() => {
    ...
    const grouped = researchData.reduce((acc, d) => {
      ...
      if (data.M && data.F) {
        ...
        if (country !== 'EU27_2020') {
          ratios[key] = {
            country,
            year,
            alcoholRatio: parseFloat(data.M.alcohol_rate) / parseFloat(data.F.alcohol_rate),
            suicideRatio: parseFloat(data.M.suicide_rate) / parseFloat(data.F.suicide_rate),
            accidentRatio: parseFloat(data.M.accident_rate) / parseFloat(data.F.accident_rate)
          };
        }
      }
    });
  ...
  correlationData: (() => {
    ...
    return {
      ...
      maleAlcoholRange: {
        min: Math.min(...maleData.map(d => parseFloat(d.alcohol_rate))),
        max: Math.max(...maleData.map(d => parseFloat(d.alcohol_rate))),
        avg: maleData.reduce((sum, d) => sum + parseFloat(d.alcohol_rate), 0) / maleData.length
      },
      maleSuicideRange: {
        min: Math.min(...maleData.map(d => parseFloat(d.suicide_rate))),
        max: Math.max(...maleData.map(d => parseFloat(d.suicide_rate))),
        avg: maleData.reduce((sum, d) => sum + parseFloat(d.suicide_rate), 0) / maleData.length
      }
    };
  })()
};
  ...
  corrections: [
    {
      ...
      evidence: "Years in dataset: " + Array.from(new Set(researchData.map(d => d.year))).sort().join(', ')
    },
    {
