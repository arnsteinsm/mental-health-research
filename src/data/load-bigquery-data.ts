...
export const processedBQData: BigQueryDataPoint[] = bqData.map((row: any) => ({
  ...
  alcohol_rate: parseFloat(row.alcohol_rate || row.Alcohol_Rate || row.ALCOHOL_RATE || 0),
  suicide_rate: parseFloat(row.suicide_rate || row.Suicide_Rate || row.SUICIDE_RATE || 0),
  accident_rate: parseFloat(row.accident_rate || row.Accident_Rate || row.ACCIDENT_RATE || 0)
}));

// Verify the actual dataset
export const actualDatasetStats = {
  totalRecords: processedBQData.length,
  uniqueCountries: Array.from(new Set(processedBQData.map(d => d.country))).filter(c => c && c !== 'EU27_2020'),
  uniqueYears: Array.from(new Set(processedBQData.map(d => d.year))).filter(y => y).sort(),
  genderSplit: {
    male: processedBQData.filter(d => d.sex === 'M').length,
    female: processedBQData.filter(d => d.sex === 'F').length
  },
  yearRange: {
    start: Math.min(...processedBQData.map(d => parseInt(d.year)).filter(y => !isNaN(y))),
    end: Math.max(...processedBQData.map(d => parseInt(d.year)).filter(y => !isNaN(y)))
  }
};
