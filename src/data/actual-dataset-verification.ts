...
const years = ACTUAL_DATASET_FACTS.uniqueYears
...
const calculateActualCorrelation = () => {
  ...
  const maleAlcoholRates = maleData
    .map((d) => Number.parseFloat(d.alcohol_rate))
    .filter((r) => !isNaN(r));
  const maleSuicideRates = maleData
    .map((d) => Number.parseFloat(d.suicide_rate))
    .filter((r) => !isNaN(r));
  const femaleAlcoholRates = femaleData
    .map((d) => Number.parseFloat(d.alcohol_rate))
    .filter((r) => !isNaN(r));
  const femaleSuicideRates = femaleData
    .map((d) => Number.parseFloat(d.suicide_rate))
    .filter((r) => !isNaN(r));
...
const calculateActualGenderRatio = () => {
  ...
  const maleRates = maleData.map((d) => Number.parseFloat(d.alcohol_rate)).filter((r) => !isNaN(r));
  const femaleRates = femaleData
    .map((d) => Number.parseFloat(d.alcohol_rate))
    .filter((r) => !isNaN(r));
...
console.log(
  'Countries:',
  ACTUAL_DATASET_FACTS.uniqueCountries.length,
  ACTUAL_DATASET_FACTS.uniqueCountries
);
