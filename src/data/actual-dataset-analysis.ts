...
export const actualData: ActualDataPoint[] = bqData.map((row: any) => ({
  ...
  sex: row.sex,
  alcohol_rate: Number.parseFloat(row.alcohol_rate),
  suicide_rate: Number.parseFloat(row.suicide_rate),
  accident_rate: Number.parseFloat(row.accident_rate),
}));

// VERIFIED DATASET STATISTICS
export const actualStats = {
  totalRecords: actualData.length,
  uniqueCountries: Array.from(new Set(actualData.map((d) => d.country))).filter(
    (c) => c !== 'EU27_2020'
  ),
...
export const countryCoverage = actualStats.uniqueCountries.reduce(
  (acc, country) => {
...
export const genderRatios = actualStats.uniqueCountries
  .map((country) => {
