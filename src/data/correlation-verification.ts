...
const avgMaleAlcohol = maleAlcoholRates.reduce((sum, rate) => sum + rate, 0) / maleAlcoholRates.length;
const avgFemaleAlcohol = femaleAlcoholRates.reduce((sum, rate) => sum + rate, 0) / femaleAlcoholRates.length;
...
export const ACTUAL_DATASET_INFO = {
  ...
  yearRange: {
    start: Math.min(...researchData.map(d => parseInt(d.year))),
    end: Math.max(...researchData.map(d => parseInt(d.year)))
  },
  genderSplit: {
