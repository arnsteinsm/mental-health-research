...
export const calculateVerifiedStats = (): VerifiedStats => {
  // Dataset overview
  const countries = Array.from(new Set(researchData.map((d) => d.country))).filter(
    (c) => c !== 'EU27_2020'
  );
  const years = researchData.map((d) => Number.parseInt(d.year)).filter((y) => !isNaN(y));
  ...
  const avgMaleRate =
    maleRecords.reduce((sum, d) => sum + Number.parseFloat(d.alcohol_rate), 0) / maleRecords.length;
  const avgFemaleRate =
    femaleRecords.reduce((sum, d) => sum + Number.parseFloat(d.alcohol_rate), 0) /
  ...
  const maleAlcoholRates = maleRecords.map((d) => Number.parseFloat(d.alcohol_rate));
  const maleSuicideRates = maleRecords.map((d) => Number.parseFloat(d.suicide_rate));
  const femaleAlcoholRates = femaleRecords.map((d) => Number.parseFloat(d.alcohol_rate));
  const femaleSuicideRates = femaleRecords.map((d) => Number.parseFloat(d.suicide_rate));
  ...
  return {
    ...
    genderRatio: {
      value: Number.parseFloat(genderRatio.toFixed(1)),
      calculation: `${avgMaleRate.toFixed(2)} ÷ ${avgFemaleRate.toFixed(2)} = ${genderRatio.toFixed(1)}`,
      methodology:
        'Average male alcohol mortality rate divided by average female alcohol mortality rate across all available data points',
    },
    correlation: {
      ...
      methodology:
        'Pearson correlation coefficient calculated using standard formula: r = Σ[(xi - x̄)(yi - ȳ)] / √[Σ(xi - x̄)² × Σ(yi - ȳ)²]',
      interpretation:
        'Values range from -1 (perfect negative correlation) to +1 (perfect positive correlation). Values above 0.7 are considered strong positive correlations.',
    },
  };
};
