...
// Convert structured data back to flat format for SQL
const flattenData = (structured) => {
  const flattened = [];
...
const generateSQL = (data) => {
  ...
  insertStatements.push('-- Structure: Country → Year → Gender → Rates');
  ...
  insertStatements.push("SELECT * FROM mental_health_data WHERE country = 'AT' AND year = 2022 ORDER BY sex;");
  
  return insertStatements.join('\n');
};

// Flatten the structured data
const flatData = flattenData(structuredData);

// Generate the SQL
const sql = generateSQL(flatData);

// Write to file
const outputPath = path.join(__dirname, 'mental-health-structured-migration.sql');
...
console.log('✅ SQL migration file generated from structured data!');
...
console.log('2. Copy and paste the contents of mental-health-structured-migration.sql');
